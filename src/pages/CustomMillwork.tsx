...
const CustomMillwork = () => {
  ...
  useEffect(() => {
    if (category && refs.current[category]) {
      ...
      <section className="py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          {millworkTypes.map((item, i) => (
            <div
              key={item.slug}
              ref={(el) => (refs.current[item.slug] = el)}
              className={`scroll-mt-24 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${i > 0 ? "mt-20 lg:mt-28" : ""}`}
            >
